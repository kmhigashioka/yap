...
describe('Transactions', () => {
  runOnSizes((size: string) => {
    ...
    it('should add an expense and reset form', () => {
      const transaction = {
        description: 'Load',
        date: '11/22/2019',
        category: {
      ...
      cy.findByTestId('select-category').should('have.value', '');
      cy.findByPlaceholderText('Description').should('have.value', '');
      cy.findByText('Add Transaction').should('be.visible');
    });

    it('should add an expense and close form', () => {
      const transaction = {
        description: 'Load',
        date: '11/22/2019',
        category: {
      ...
      cy.findByText('Transaction successfully created.').should('be.visible');
      cy.findByText('Add Transaction').should('not.exist');
    });
    ...
    it('should edit first transaction and reset form', () => {
      ...
      cy.get('[data-testid="transaction-row-id-1"]').then((subject) => {
        ...
        cy.findByText(newTransaction.description, {
          container: subject,
        }).should('be.exist');
    ...
    it('should reset form when Edit transaction is cancelled', () => {
      ...
      cy.findByPlaceholderText('Date').should(
        'have.value',
        oldTransaction.date,
      );
    });
